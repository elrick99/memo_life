...
const _table = 'saving_goals';

class SavingGoalLocalDataSource {
  SavingGoalLocalDataSource(this._appDatabase);
  ...
  Future<List<SavingGoalModel>> getAll() async {
    ...
    return rows.map(SavingGoalModel.fromRow).toList();
  }

  Future<SavingGoalModel?> getByLocalUuid(String localUuid) async {
    final db = await _appDatabase.database;
    ...
    return rows.isEmpty ? null : SavingGoalModel.fromRow(rows.first);
  }

  String newLocalUuid() => _uuid.v4();

  Future<void> upsert(SavingGoalModel goal) async {
    final db = await _appDatabase.database;
    await db.insert(
      _table,
      goal.toRow(),
